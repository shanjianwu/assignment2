...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    # the out variable.                                                         #
    #############################################################################
    x_normarized = (x - running_mean)/np.sqrt(running_var+eps)
    out = gamma * x_normarized + beta
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
  else:
...
def batchnorm_backward(dout, cache):
  ...
  dsample_var = -0.5*np.sum(dx_normalized * x_mu, axis=0, keepdims=True)
    ...
    for f in range(F):  # fth filter
      for j in range(H_new):
        for k in range(W_new):
          out[i, f, j, k] = np.sum(x_padded[i, :, j * s:HH + j * s, k * s:WW + k * s])
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  cache = (x, w, b, conv_param)
